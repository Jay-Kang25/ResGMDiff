...
def _list_image_files_recursively(data_dir,endwith):
    """
    Recursively list all image files in a directory, returns full paths.
    Filters for files ending with 'target.npy'.
    """
    results = []
    for entry in sorted(bf.listdir(data_dir)):
        full_path = bf.join(data_dir, entry)
        if bf.isdir(full_path):
            results.extend(_list_image_files_recursively(full_path))  # Recursively search subdirectories
        elif entry.endswith(endwith):  # Check if the file ends with 'target.npy'
...
class Dataset(Dataset):
    def __init__(
        ...
        if condition == 1:
            # condition
            self.gt = _list_image_files_recursively(folder[0], 'target.npy')
            self.input = _list_image_files_recursively(folder[1], 'input.npy')

        elif condition == 2:
            self.gt = _list_image_files_recursively(folder[0], 'target.npy')
            self.input = _list_image_files_recursively(folder[1], 'input.npy')
            self.input_condition = _list_image_files_recursively(folder[2], 'input_condition.npy')
    ...
    def __getitem__(self, index):
        if self.condition == 1: #we use the condition 1
            # condition
            img0 = np.load(self.gt[index])
            img1 = np.load(self.input[index])
            w, h = img0.shape
            ...
            img0 = torch.FloatTensor(img0.copy()).unsqueeze_(0)
            img1 = torch.FloatTensor(img1.copy()).unsqueeze_(0)
            ...
            img0 = np.load(self.gt[index])
            img1 = np.load(self.input[index])
            img2 = np.load(self.input_condition[index])
            ...
            img0 = torch.FloatTensor(img0.copy()).unsqueeze_(0)
            img1 = torch.FloatTensor(img1.copy()).unsqueeze_(0)
            img2 = torch.FloatTensor(img2.copy()).unsqueeze_(0)
