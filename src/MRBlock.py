...
class MRBlock(nn.Module):
    def __init__(self, in_channel ,out_channel, num_res=3):
        super(MRBlock, self).__init__()
        layers =  []
        layers.append(ResBlock(in_channel, out_channel))
        layers.extend([ResBlock(out_channel, out_channel) for _ in range(num_res-2)])
...
class ResBlock(nn.Module):
    def __init__(self, in_channel, out_channel, filter=False):
        ...
        self.res_conv = nn.Conv2d(
            in_channel, out_channel, 1) if in_channel != out_channel else nn.Identity() #这一个原来没有
        self.main = nn.Sequential(
            BasicConv(in_channel, out_channel, kernel_size=3, stride=1, relu=True),
            MBA(out_channel, out_channel) if filter else nn.Identity(), #这里给改了
...
class MBA(nn.Module):
    def __init__(self, k, k_out):
        ...
        for j, i in enumerate(self.pools_sizes):
            ...
            dynas.append(MAA(dim=k, kernel_size=3, dilation=dilation[j]))
...
class MAA(nn.Module):
    def __init__(self, dim, kernel_size=3, dilation=1, group=8):
        ...
        self.square_att = GAB(inchannels=dim, dilation=dilation, group=group, kernel_size=kernel_size)
        self.strip_att = CAB(dim, group=group, dilation=dilation, kernel=kernel_size)
